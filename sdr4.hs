...
hardAddresses :: M.Map [Word8] [Int]
hardAddresses = M.fromList [( [0,0,0,0], wordsToCounters ([0,0,0,1]::[Word8])),
                            ( [1,1,1,1], wordsToCounters ([1,1,1,1]::[Word8])),
                            ( [2,2,2,2], wordsToCounters ([2,2,2,2]::[Word8])),
                            ( [4,4,4,4], wordsToCounters ([4,4,4,4]::[Word8]))]

-- Filterkey based on standard hamming distance. Gives returned addresses, necessary for read/write ops.
findAddresses :: M.Map [Word8] [Int] -> [Word8] -> M.Map [Word8] [Int]
...
--Write a new value(counters) to a memory
writeCounters :: [Word8] -> [Int] -> M.Map [Word8] [Int] -> M.Map [Word8] [Int]
writeCounters = M.insertWith listSum


--Problem with using insert with: Type ends up having to be Int going in. I would expect to have to give it a [Word8], but that wouldn't match the internal
--type of the counters.
writeValue :: [Word8] -> [Word8] -> M.Map [Word8] [Int] -> M.Map [Word8] [Int]
writeValue address word sdm = M.mapWithKey (f word) sdm 
	where f = \c k a -> if (k `hamming'` address <= range)
				then (listSum (wordsToCounters (c::[Word8]) ) a)
				else a
